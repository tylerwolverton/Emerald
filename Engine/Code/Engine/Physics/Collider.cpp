...
typedef Manifold2D( *CollisionManifoldGenerationCallback )( const Collider*, const Collider* );
...
//-----------------------------------------------------------------------------------------------
Manifold2D Collider::GetCollisionManifold( const Collider* other ) const
{
	if ( other == nullptr
		 || !m_rigidbody->IsEnabled()
		 || !other->m_rigidbody->IsEnabled()
